---
title: "Intelligence per Watt: Local AI Efficiency on the rise"
paper_authors: "Jon Saad-Falcon, et al."
orgs: "Standford University, Together AI"
...
image_dir: "/assets/images/posts/2025-11/potm/ipw/"
review_author:
...
<img src="{{ page.image_dir | append: 'fig1.png' | relative_url }}" class="constrained_img_large" style="max-width: 700px" alt="Figure1 - Increase in Intelligence per Watt for Local LMs and Accelerators">
...
**Hybrid local-cloud inference saves energy.** Perhaps the most practical finding is that a smart routing system can yield enormous resource savings. If an oracle router always chooses the smallest adequate local model for each query (and falls back to a big cloud model only when needed), the analysis showed a ~80% reduction in energy consumption and similar drops in compute and cost, compared to using cloud-only LLMs. Even a more realistic router (e.g. one that guesses correctly 80% of the time) can cut total energy use by around 60% without hurting accuracy. Given the current challenge in building AI inference capabilities globally, shifting load to local devices is an interesting proposition.

### Limitations

This paper is a good first step in figuring why and how local AI inference capabilities in devices should be prioritized. That being said the system are measured running fairly simple scenarios that may not reflect real-world usage. 
