...
![Lines emanating from a central point (0, 0). A bold line labelled 'a'. A line in the same direction, but twice as long, labelled 'x . a = 2'. A sequence of lines rotating from those, labelled 'x . a = 1/sqrt(2)' then 'x . a = 0' then 'x . a = -1/sqrt(2)', then 'x . a = -1' in the opposite direction from a.]({{ page.image_dir | append: 'dot_product.png' | relative_url }}){:class="constrained_img_small"}
...
![a grid of coloured squares representing cosine similarity between embeddings; "red" and "green" are similar; "London", "Paris" and "Tokyo" are similar; "pleasant" and "vibrant" are similar; "with" really is on it's own]({{ page.image_dir | append: 'embeddings.png' | relative_url }}){:class="constrained_img_small"}
...
![A plot of blue and orange dots for the attention and mlp layer's update scales. Most points hover around the 0.05 mark, with the first layer having a big attention update, and the last layer having a big mlp update.]({{ page.image_dir | append: 'residual_updates.png' | relative_url }}){:class="constrained_img"}
...
![A log-y linear-x plot of scale with and without RMSNorm. The with-norm line grows slowly and remains under 10^2 for all 18 layers, while the without-norm line grows quickly, exploding to 10^20 after just 3 layers.]({{ page.image_dir | append: 'norm_scaling.png' | relative_url }}){:class="constrained_img"}
...
![A grid of 5 by 5 pixel images, showing lower-left triangles filled with various patterns. Some are dense (all pixels dark), some are sparse, usually with either a diagonal dark stripe or one on the far left.]({{ page.image_dir | append: 'attention.png' | relative_url }}){:style="width: 100%;"}
...
![5 vectors, labelled k_0 to k_4, the first pointing right along the x-axis, the others rotated at regular intervals about (0, 0).]({{ page.image_dir | append: 'rotary_example.png' | relative_url }}){:class="constrained_img"}
...
![A straight line on a linear x-axis of component, ranging from 0 to 128, and a log y-axis of wavelength, ranging from 4 to 65536. And a horizontal line at 8192 labelled "max sequence length".]({{ page.image_dir | append: 'rotary_wavelength.png' | relative_url }}){:class="constrained_img_small"}
...
![3D surface plot showing a flat slope.]({{ page.image_dir | append: 'mlp_linear.png' | relative_url }}){:class="constrained_img_small"}
...
![Plot of z = relu(a), with a flat portion below zero, then a linear portion above zero.]({{ page.image_dir | append: 'relu.png' | relative_url }}){:class="constrained_img_small"}
...
![A pinwheel map of colours in different triangular segments: green then gray then blue then magenta then off-white then yellow.]({{ page.image_dir | append: 'mlp_zmap.png' | relative_url }}){:class="constrained_img_small"}
...
![3D surface plot showing a piecewise linear function of two input components. Each piece is coloured as per the pinwheel map above.]({{ page.image_dir | append: 'mlp_relu.png' | relative_url }}){:class="constrained_img_small"}
...
![3D surface plot showing a piecewise quadratic function of two input components.]({{ page.image_dir | append: 'mlp_reglu.png' | relative_url }}){:class="constrained_img_small"}
...
![A plot of GELU(a) and ReLU(a). From a=-2 to a=2, GELU starts near zero, then drops before reaching z=0 when a=0 before climbing to approach z=a.]({{ page.image_dir | append: 'gelu.png' | relative_url }}){:class="constrained_img_small"}
...
![3D surface plot showing a smooth function of two input components.]({{ page.image_dir | append: 'mlp_geglu.png' | relative_url }}){:class="constrained_img_small"}
...
![Four line plots, shown as x[0] varies from -2 to 2. The first, linear is linear. The second, ReLU, is piecewise linear. The third, ReGLU, is piecewise quadratic with gradient discontinuities. The fourth, GeGLU, is smooth but still vaguely quadratic.]({{ page.image_dir | append: 'mlp_slice.png' | relative_url }}){:class="constrained_img"}
...
![A plot of prediction probability after each term in the sequence "&lt;bos&gt; I want to move". The predictions have different tokens (e.g. "I" followed by "have") and probabilities.]({{ page.image_dir | append: 'predictions.png' | relative_url }}){:class="constrained_img_large"}
