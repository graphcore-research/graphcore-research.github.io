...
- training-dynamics
potm_order: 1
paper_title: Scaling Exponents Across Parameterizations and Optimizers
paper_authors: Katie Everett, et al.
paper_orgs: Google DeepMind, MIT
...
![Learning rate transfer over width for Adam for each of the four parametrisations when assuming full alignment, showing good transfer for all except MFP.](./figure4-adam-full-transfer.png)

Compare this with the **no alignment** assumption, which doesn't give good transfer with plain Adam:

![Learning rate transfer over width for Adam for each of the four parametrisations when assuming no alignment, showing poor transfer - the base LR should be reduced for large model sizes.](./figureE1-adam-noalign-transfer.png)
...
![Scaling curves for various parametrisations, under the default epsilon scheme $10^{-9}$, smaller epsilon $10^{-15}$, per-layer epsilon with base $10^{-12}$ and their atan2 variation.](./figure7-adam-epsilon.png)
