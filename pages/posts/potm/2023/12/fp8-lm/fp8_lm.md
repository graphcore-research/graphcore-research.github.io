...
- number-formats
potm_order: 1
paper_title: 'FP8-LM: Training FP8 Large Language Models'
paper_authors: Houwen Peng, Kan Wu, Yixuan Wei, et al.
paper_orgs: Microsoft Azure and Microsoft Research
...
![A plot with Model Size (B) on the x-axis and Number of GPUs on the y-axis. Two lines showing BF16 and FP8. BF16 requires more GPUs for a given model size vs FP8 (curve underneath).](./figure_1.png)
...
![Image showing 4 bytes for master weights, 4 for gradients, 4+4 for Adam states.](./fp32_optim.png)

They suggest that the following mix of FP16/8 is viable without degradation:

![Image showing 2 bytes for master weights, 1 for gradients, 1+2 for Adam states.](./fp8_optim.png)
...
![Plot showing that training loss of FP8 matches FP16 for GPT 7B, 13B and 175B. Below, table showing similar downstream task performance of FP8 vs BF16.](./figure_4.png)
...
![Table showing that their FP8 implementation has better throughput and lower memory usage than BF16 (by far) and FP8 with transformer engine (by less) across model sizes.](./table_5.png){:style="padding-left:50px; padding-right:50px"}
